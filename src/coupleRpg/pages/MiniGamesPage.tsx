    ...
    <div className="pb-2">
      ...
      <div className={`mb-3 rounded-2xl border border-rose-100/90 bg-white/95 px-3 py-2.5 ${lq.card}`}>
        ...
        <p className="mt-0.5 text-[10px] leading-snug text-stone-500">
          {atCap
            ? '今日獎勵已達上限，仍可繼續玩。'
            : `完成一次可獲 LoveCoin +2、愛心 +3、默契 +1、EXP +5（每日最多 ${MAX_DAILY_REWARDS} 次）。`}
        </p>
      </div>
      ...
      <section className={`rounded-2xl border border-rose-100/80 p-3.5 ${lq.card}`}>
        ...
        {!line ? (
          <div className="py-6 text-center">
            <p className="mb-4 text-[12px] text-stone-500">點下方按鈕，抽一個今天的小驚喜</p>
            <button type="button" onClick={draw} className={`rounded-xl px-5 py-2.5 text-sm font-bold ${lq.btnPrimary}`}>
              {modeDef.actionLabel}
            </button>
          </div>
        ) : (
          <div className="space-y-4">
            ...
            {roundRewarded && lastGrantOk !== null ? (
              <p className="text-center text-[11px] leading-snug text-stone-500">
                {lastGrantOk
                  ? '已發放獎勵：LoveCoin +2、愛心值 +3、默契度 +1、EXP +5。'
                  : '今日獎勵已達上限，這次沒有額外點數，謝謝你們的陪伴！'}
              </p>
            ) : null}
          </div>
        )}
